...
- `src/chemistry/store.kotoba` — `Store` protocol + `MemStore`:
  registered samples, test protocols, equipment, committed records, an append-only audit ledger.
- `src/chemistry/advisor.kotoba` — `Advisor` protocol; `mock-advisor`
...
- `src/chemistry/governor.kotoba` — `LabTechnicianGovernor/check`: a pure
  ...
  human sign-off.
- `src/chemistry/actor.kotoba` — `build-graph`, `run-request!`,

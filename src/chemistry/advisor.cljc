(ns chemistry.advisor
  "LabAdvisor — proposes a lab operation (run a test protocol on a sample,
  log a result, flag an out-of-spec result, calibrate equipment) for a
  registered sample. The advisor is swappable: `mock-advisor`
  ...
  PROPOSAL — it never writes to the store and has no notion of sample
  provenance or analytical integrity risk; `chemistry.governor` is the
  independent system that decides whether the proposal may proceed, per
  the itonami actor pattern.

  A proposal is a map:
    {:op :run-test-protocol|:log-result|:flag-out-of-spec-result|:calibrate-equipment
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for sample " (:sample-id request))})
...
(def ^:private system-prompt
  "You are a lab technician advisor for chemical and physical science
   testing. Given a lab operation request, propose an :op, an honest

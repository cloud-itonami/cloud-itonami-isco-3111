(ns chemistry.actor
  "LabActor — the ISCO-08 3111 lab support actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one lab operation request
  ...
  The unconditional invariant: the LabAdvisor can never
  directly commit a record or dispatch a robot action the
  LabTechnicianGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [chemistry.advisor :as advisor]
            [chemistry.governor :as governor]
            [chemistry.store :as store]))

(defn build-graph
  "Build a compiled LabActor graph. `store` implements
  `chemistry.store/Store`. `advisor` implements
  `chemistry.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:sample-id (:sample-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
...
(defn run-request!
  "Run one lab operation request to completion or interrupt.
